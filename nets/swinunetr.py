...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        # deep supervision heads (nnUNet-style)
        self.out_ds = nn.ModuleList([
            UnetOutBlock(spatial_dims=spatial_dims, in_channels=8 * feature_size, out_channels=out_channels),  # for dec3 (/16)
            UnetOutBlock(spatial_dims=spatial_dims, in_channels=4 * feature_size, out_channels=out_channels),  # for dec2 (/8)
            UnetOutBlock(spatial_dims=spatial_dims, in_channels=2 * feature_size, out_channels=out_channels),  # for dec1 (/4)
            UnetOutBlock(spatial_dims=spatial_dims, in_channels=1 * feature_size, out_channels=out_channels),  # for dec0 (/2)
        ])
    ...
    def forward(self, x_in):
        ...
        # deep supervision logits at lower resolutions
        logits_ds_16 = self.out_ds[0](dec3)  # /16
        logits_ds_8  = self.out_ds[1](dec2)  # /8
        logits_ds_4  = self.out_ds[2](dec1)  # /4
        logits_ds_2  = self.out_ds[3](dec0)  # /2
        
        # (B, out_channels, D, H, W) full-res
        logits_full = self.out(out)  

        return [logits_full, logits_ds_2, logits_ds_4, logits_ds_8, logits_ds_16]
